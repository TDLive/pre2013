...
class TrippingRobotSettings {
	#Prefix: Where *.trippingrobot files should be stored with ending slash, put "./" for current directory, NOT RECOMMENDED
	function prefix() { return "/cosites/gir/trippingrobot/alpha_data/"; }
...
<html>
	...
	<body>
		<p align="center"><center>
			<img src="trlogo.png" height="100" width="100"/>
			<font face="Comic Sans MS"><h1>TrippingRobot</h1></font><br>
			<?php if(@isset($error)) echo "<font color='red'>$error</font><br>";
			if(@isset($success)) echo "<font color='blue'>$success</font><br>"; ?>
			<b>Welcome to <?php echo TrippingRobotSettings::sitename(); ?>!</b><br><br>
	...
	<p align="center"><center>
			<p align="center"><center>TrippingRobot &copy;2012-2016 TDLive.org Inc.<br> This site is independently owned and operated by <?php if(! TrippingRobotSettings::siteURL() == null){ echo "<a href='" . TrippingRobotSettings::siteURL() . "'>"; } echo TrippingRobotSettings::sitename(); if(! TrippingRobotSettings::siteURL() == null){ echo "</a>"; } ?>.<br> TrippingRobot is available <a href="http://github.com/TDLive/trippingrobot/">for free</a> under the <a href="http://www.gnu.org/licenses/gpl">GNU GPL</a>.</center></p>
	</center></p>
</html>
<?php
	}
	...
	function __construct(){
		if($_GET['page'] == "signup"){
			if(! @isset($_POST['user']) || ! @isset($_POST['pass'])){
				TrippingRobotPages::loginPage("Please fill out both fields.", null);
				exit;
			}
			if(! TrippingRobotCore::createTrippingRobot($_POST['user'], $_POST['pass'])){
				TrippingRobotPages::loginPage("There was an error creating your account: that username's already been used!", null);
				exit;
			}
			TrippingRobotPages::loginPage(null, "Thanks for creating an account! You can now login using the form below.");
			exit;
		}
		if($_GET['page'] == "logout"){ #logging out?
			setcookie ("username", "", time() - 3600);
			setcookie ("password", "", time() - 3600);
			TrippingRobotPages::loginPage(null, "Logged out.");
			exit;
		}
		if($_GET['page'] == "rcinput"){
			$settings= new TrippingRobotSettings;
			...
			if(! is_readable($settings->prefix() . $_COOKIE['username'] . ".trippingrobotlogin")){
				TrippingRobotPages::loginPage("Invalid username!", null);
				exit;
			}
			...
			if(! $storedpass=file_get_contents($settings->prefix() . $_COOKIE['username'] . ".trippingrobotlogin")){
				TrippingRobotPages::loginPage("Internal server error. Please wait a couple of minutes, and try again.", null);
				exit;
			}
			#Passwords are md5() ed with a salt, so let's encode the current password.
			$password=$_COOKIE['password'];
			if($password != $storedpass){
				TrippingRobotPages::loginPage("Invalid password!", null);
...
<th>Awards (if any)</th>
		...
		if(@isset($_COOKIE['username']) && @isset($_COOKIE['password'])){ #we hav cookiehs!
			...
			#Does the user even exist/can we read the user's profile?
			if(! is_readable($settings->prefix() . $_COOKIE['username'] . ".trippingrobotlogin")){
				TrippingRobotPages::loginPage("Invalid username/password!", null);
				exit;
			}
			#Let's store the password, if we can't let's tell the user we can't.
			if(! $storedpass=file_get_contents($settings->prefix() . $_COOKIE['username'] . ".trippingrobotlogin")){
			...
			TrippingRobotPages::homePage($_COOKIE['username']);
			exit;
		}
		if(@isset($_GET['login'])){ #logging in?
			#YES, is errythin set?
			if(! @isset($_POST['user']) || ! @isset($_POST['pass'])){
				#tell the user that they must do both a username/pass
				TrippingRobotPages::loginPage("Please define both a user and password.", null);
				exit;
			}
			$username=$_POST['user'];
			...
			TrippingRobotPages::homePage(USER);
			exit;
		}
		else{ #show us the login page
			TrippingRobotPages::loginPage(null, null);
		}
	}
}
